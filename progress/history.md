...
## Sesión 2026-08-10 — ui-design-tokens (id: 23)

- **Origen:** el usuario pidió ayuda para redactar un prompt de la skill `ui-ux-pro-max` porque el sistema "se veía muy básico". La sesión derivó en generar el sistema de diseño completo y ejecutar la primera feature del refactor.
- **Diagnóstico:** cuatro causas concretas, ninguna de calidad de código. `--primary` acromático `oklch(0.205 0 0)` (default de shadcn sin tematizar); `--card` y `--background` ambos `oklch(1 0 0)`, sin jerarquía de superficie; radios de app de consumo (`rounded-2xl` por todas partes); y `@fontsource-variable/inter` + `geist` en `package.json` **sin un solo import en `frontend/src/`**, así que la app renderizaba con Segoe UI.
- **Sistema de diseño:** `design-system/odc/MASTER.md` + 4 overrides de página, generados con `ui-ux-pro-max` contra el tipo de producto "Invoice & Billing Tool" (`products.csv` row 105) con dials variance 3 / motion 3 / density 8. Cuatro recomendaciones descartadas con motivo registrado: el bloque STYLE (devolvió "Exaggerated Minimalism" —fashion, portfolios, luxury— con tres queries distintas, misruteo del ranker; se usó la fila autoritativa de producto), el pairing Fira Code/Fira Sans (Inter ya instalada y es el pairing "Minimal Swiss" de la propia DB para admin panels), el preset GSAP + ScrollTrigger (view transitions y keyframes propios ya cubren el tier Subtle) y los iconos Phosphor (Lucide ya en uso).
- **Conflicto de gobernanza:** existía `/DESIGN.md` (generado por `impeccable`) con 7 reglas que contradecían al MASTER. Decisión humana: MASTER es fuente única. `/DESIGN.md` **no** se abandonó —la skill `impeccable` lo lee automáticamente vía `scripts/context.mjs`, así que dejarlo intacto habría seguido inyectando las reglas viejas— sino que se sincronizó: frontmatter y cuerpo actualizados, banner "derivado, no editar a mano". La `Status-Only Color Rule` (cero acentos de marca) se sustituyó por la **Two-Color Rule**: navy para chrome/acción/foco, hues de estado para el ciclo, nada más.
- **Defecto de spec detectado en implementación:** R2 y R5 eran **contradictorios tal como se aprobaron**. R2 fijaba `--accent-action-foreground` blanco sobre `--accent-action` `#059669`, combinación que da 3.77:1; R5 exigía ≥4.5:1. No existía implementación que cumpliera ambos. El `spec_author` no lo detectó al redactarlos. El `implementer` chocó con ello, eligió R5 (cambiando el foreground a navy, 4.74:1) y lo escaló en vez de silenciarlo. Decisión humana: corregir por el lado del verde. `--accent-action` pasa a `oklch(0.5081 0.1049 165.61)` (#047857, 5.48:1 con blanco), valor que además ya era `--status-done`, de modo que el botón de confirmar y el estado al que conduce comparten verde. Registrado como **enmienda firmada** en `requirements.md` (sección propia + casilla fechada), no como edición en sitio, porque C6 exige que todo cambio de requisito posterior a la aprobación muestre su gate.
- **Nota de proceso (gate humano):** por primera vez en el proyecto el `spec_author` **no** se auto-marcó la casilla — la dejó en `[ ]` y el leader lo verificó de forma independiente antes de continuar. El humano la marcó a mano. Rompe la racha de 6 recurrencias (memoria `spec-author-checkbox-unreliable`); el refuerzo del role prompt parece haber funcionado.
- **Nota de proceso (C4):** también por primera vez el `implementer` respetó test→feat sin necesidad de reescribir el historial. 11 commits, ningún par mezclado, verificado por el leader y por el reviewer archivo por archivo y con rojo empírico en worktree aislado (55/77, 12/18, 3/78). Rompe la racha de 3 rechazos por C4 (memoria `implementer-bundles-test-impl-commits`).
- **Trampa evitada:** el rol del `implementer` ordena usar el MCP de shadcn para componentes de UI. Aquí habría sido destructivo: las primitivas de `ui/` están adaptadas a `@base-ui/react`, no a Radix, y regenerarlas habría borrado esa adaptación. Se le instruyó explícitamente a editar los archivos existentes.
- **Revisión:** aprobada con dos defectos, ambos de documentación y ambos corregidos por el leader. D1: `traceability.md` e `impl_*.md` afirmaban que el commit de test de R13/R15 precedía al `feat` que guardan — falso, `81f7ee7` es posterior a `e9401d9` y `31c88a7`; solo R14 cumple el orden. Sin impacto material (la guarda pasa 18/18 sobre el árbol pre-feature, es verde a ambos lados) pero `traceability.md` es registro auditable y no puede contradecir un `git log`. D2: la nota de enmienda —escrita por el leader— citaba R2 en vez de R3 y leía la restricción de chroma al revés: es un techo de 0.10 del que `--accent-action` está exento por nombre, no un suelo que el valor cumpla.
- **Puntos frágiles verificados por el reviewer, los tres resisten:** la exención de chroma la concede R3 tal como se aprobó y el regex del test es transcripción literal presente desde `aa31027`, anterior al conflicto; la composición de alpha del test de contraste se hace en sRGB codificado, que es lo que hace el navegador (rasterizado a 8 bits: 5.279 frente a 3.986 en lineal); y los 26 pares de R5 recalculados con conversor independiente y gamut mapping real van de 4.617 a 17.941.
- **Alcance:** solo fases 1-2 (`styles.css` + las 9 primitivas de `ui/`). Los tokens `--status-*` se declaran pero **aún no se consumen**: `odc-status-badge.tsx` es fase 3e. `ui/toast.tsx` quedó fuera y es la única superficie con el radio viejo — deuda de una línea.
- **Hallazgo colateral:** `general-approval-actions.test.tsx:163` es flaky preexistente, ajeno a esta feature. Solo falla con la suite completa por una carrera de render (`waitFor` sobre la desaparición del botón seguido de un `getByText` síncrono del texto de estado). Guardado en memoria de Claude como `flaky-general-approval-test`.
