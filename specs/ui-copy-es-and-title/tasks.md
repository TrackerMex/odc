...
- [ ] (1) Escribir test que falla para R1: en `login-form.test.tsx`, afirmar que
      `getByLabelText('Correo electrónico')` devuelve el input `id="email"` y
      `getByLabelText('Contraseña')` el input `id="password"`
- [ ] (2) Implementación mínima que lo pasa: los dos `FieldLabel` de
      `login-form.tsx` (líneas 109 y 142)
- [ ] (3) Refactor con tests verdes: actualizar las queries vigentes que rompen —
...
- [ ] (1) Escribir test que falla para R2: afirmar el literal
...
- [ ] (2) Implementación mínima que lo pasa: mensaje por regla en
      `src/lib/login-schema.ts`
- [ ] (3) Refactor con tests verdes: comprobar que los tests preexistentes de
      R7 (`textContent !== ''`) siguen pasando y no quedan duplicados con los nuevos

## R3 — `<title>` del documento

- [ ] (1) Escribir test que falla para R3: leer el `head()` de la ruta raíz
      (`src/routes/__root.tsx`) y afirmar que su `meta` contiene exactamente
      `ODC — Órdenes de compra`
- [ ] (2) Implementación mínima que lo pasa: sustituir el `title` de
      `__root.tsx:26`
- [ ] (3) Refactor con tests verdes: confirmar que la cadena
