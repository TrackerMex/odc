...
| R2 | `backend/src/modules/odc/domain/entities/purchase-order.entity.spec.ts::R2: totalCents computed in the domain, never accepted from outside`; `backend/src/modules/odc/application/use-cases/create-draft.usecase.spec.ts::R2: a totalCents smuggled into the create payload never reaches the ODC` | `76783c1` feat(odc): add pure PurchaseOrder domain entity with computed total (R1,R2); `29d02bd` feat(odc): add POST /api/odcs creating a draft for DIRECTOR_OPS (R7) |
